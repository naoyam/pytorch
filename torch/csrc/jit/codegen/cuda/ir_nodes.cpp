...
class BroadcastMapping: public BackwardVisitor {
 public:
  ...
  void handle(BinaryOp* bop) override {
    if (bop->output(0)->getValType() != ValType::TensorView) return;
  ...
  void handle(UnaryOp* uop) override {
    if (uop->output(0)->getValType() != ValType::TensorView) return;
  ...
  void handle(ReductionOp* rop) override {
    if (rop->output(0)->getValType() != ValType::TensorView) return;
  ...
  void handle(BroadcastOp* op) override {
    if (op->output(0)->getValType() != ValType::TensorView) return;
